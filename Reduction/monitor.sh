...
do
	...
	for batch in `echo "mc_radion_signal data2012_RERECO mc_Summer12_RD1"`
	do
		./check_status.py ${batch} > ${batch}_status.txt
		cat ${batch}_status.txt | grep "submit_reduction.sh"
		eval `cat ${batch}_status.txt | grep "submit_reduction.sh"`
	done
